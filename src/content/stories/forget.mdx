...
In a strict sense, can we say remembering is the ability to recall a piece of information after having experience it once?
...
Given two coins, each coin has two sides: Head (H) or Tail (T).
...
The result of a coin toss is $T$ out of $H, T$? Then we collapsed the probability from 1/2 to 1.

This is the ratio of 2.

---

The result of two coin tosses was $TH$ out of four possibilities HH, HT, TH, TT? Then we collapsed the probability from 1/4 to 1.
...
Entropy is the measurement of uncertainty. If an event with a probability p(x) happens, you would know -log(p(x)) bits of information. Summing that up for all events, weighed by how rare each event is, you have entropy.

---

For a coin toss, information you gain is 2 bits for 4 possibilities each with a probability of 1/4.
...
Knowing or not knowing the result of a coin toss, it doesn't affect the actual coin. It'd not simply goes back to spinning just because you didn't know it.
...
Picking 2 out of 16 cards **out of random** and it's a match means a 1/15 chance and you collapse that to 1.
...
So now the possibility of 1 exploded back to 1/4.
...
Because knowledge doesn't directly equate behavioral patterns, it simply passes information to the next "layer" to decides.
