...
import { GuessBox, StatChart } from "@/components/stories/proverbs/guess-box.tsx";
...
<TicTacToe client:visible state={["X", "O", "X", "O", "X", null, "X", null, "O"]} />
...
<ChessBoard client:visible fen={"rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2"} />
...
<span class="text-xs">
  (at least in Lagrangian mechanics, there are a few different _religions_ of physics that have
  different definitions of action)
</span>
...
<span class="text-xs">
  In reality, this is not what superposition means, but this is the limit of analogy too, so just go
  with it.
</span>
