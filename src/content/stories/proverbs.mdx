...
Isn't it strange? If there are two sides of the coin every time, does it even matter?
...
> Block your opponent's move when you see two line up.
...
Chess, on the other hand, is far more complex. Just two moves ahead, the combinations already exponentially explode.
...
Subversion makes you question your assumptions and world view.
...
In decision theory, there are **Risk** and there are **Uncertainty**.

If we say certainty is that we know the only possible outcome of an event,
...
We just has enough empirical evidence to support it.

Doesn't mean it is an absolute truth, but it just doesn't make sense to ignore it.

---

That makes some proverbs more popular and widely accepted than its contradictory anti-proverb.
...
Or they luckily got a counterexample that opened their eyes to a newfound truth that they want to share with the world.
...
Most people tend to prefer things that align with our belief system if our belief system also "prefer things" that align with our preferences.
...
The latter part serves as an explanation for how come we change in a non-continuous, contradictory way.
...
And also happens to trust our definition of arithmetic enough to say that
...
The reason why it doesn't make so much sense because we are stating the law as if it is a **prescriptive** law.
...
Usually the most law-breaking counterexamples are usually those who are really really fast, or really really small.
...
A more accurate analogy would be a line of a marching band going through a boundary of concrete road and mud.
...
The band practices for countless of time, they don't have to care about the time so it simply just tries marching in every possible angle.

Intuitively, they tried to go in a straight line.
...
When the leftmost line stomps slower, they go out of sync. Left, Right, Left, Right coincides with Right, Left, Right, Left.
...
It just so happens that for every angle they tried, the phases got out of sync, and makes the probability of observing the those angles lower and lower--infinitesimally small.
...
Eventually, there the angle that keeps them in sync has the highest probability of being observed.
...
When we say a point is "stationary", we means that the rate of change at that point is zero.

Oh boy, how do I connect this back to proverbs?

---

Meaning that if we nudge the path a little bit to the left, teeny tiny amount, there would be no change to the action.
...
Nevertheless, we have learned to find a way to approximate the solution!

---

So the question: can't we really find an algorithm that runs really fast, yet gives a solution that is really close to the optimal solution?
...
If we can solve the partition problem, then we can also solve the bin packing problem for two bins.
...
We can say that the guaranteed performance of any algorithm that approximates the bin packing problem is at least as bad as 3/2.
...
The reason why is because if there is a solution that is better than 3/2, then we can use that algorithm to solve the partition problem in polynomial time, which would imply that P = NP.

---

And that is something we don't have an answer to yet, and would break our world view if it turns out to be true.
...
We don't want to find an AGI that can solve all problems, because that would be inefficient.
...
Not to offend the ornithologists, but nobody is really keeping track whether the early bird actually catches more worms.
...
A good proverb is when it is descriptive, not prescriptive.

---

It is when it describes the patterns that we observe, not when it dictates how we should behave.

---

Sure, reality is complex and will subvert our expectations, but proverbs have **approximation ratios**.

Knowing everything with a little set of rules, with an efficient time of computation, is going to break our understanding of the world.
...
I'm not saying that you **should** learn from the mistakes by describing the patterns that you observe, instead of developing a prescriptive rule that "I should never do this again".
