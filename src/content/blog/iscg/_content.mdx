...
`MouseState` は、マウスの状態を管理するための型である。`intersect` は、頂点の 0.2 単位以内にマウスがあるときに、頂点を選択するためのものである。`picked` は、`intersect` の頂点を `picked` に格納するためのものである。それ以外 (picked でも、intersect でもない) には、マウスをクリックすると頂点を追加することになる。つまり、有限状態機械のようなものになる。
